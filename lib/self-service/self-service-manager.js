...
const GET_USER_PROFILE_BY_ID = "get user profile by id";
...
SelfServiceManager.prototype.getUserProfileById = function (uuid, iamToken) {
	let qs = "/" + uuid;
	 const deferred = Q.defer();
	 let url = this.managementUrl + MGMT_USER_PROFILES + qs + "/profile";
	 _getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then(function (token) {
			 _handleRequest(token, GET, url, {} , qs, GET_USER_PROFILES, deferred);
	 }).catch(deferred.reject);
	 return deferred.promise;
