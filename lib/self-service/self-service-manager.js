...
const GET_USER_PROFILES = "get user profiles";
...
const MGMT_USER_PROFILES = "/users";
...
/**
 * @param {string=} query, optional, query string to search for users - empty string for all profiles
 * @param {string=} start, startIndex for directory query, 0 for single user search
 * @param {string=} count, number of returned documents, 0 for single user search
 * @param {string=} iamToken, optional, if passed request to the server will use this token.
 * @return {Object} The users SCIM profiles.
 */
SelfServiceManager.prototype.getUserProfiles = function (query, start, count, iamToken) {
	let oneUser = "&dataScope=full"
	let allUsers = "?dataScope=full&startIndex=" + start + "&count=" + count;
	let qs = query !== "" ? "?email=" + query + oneUser : allUsers;
	 const deferred = Q.defer();
	 let url = this.managementUrl + MGMT_USER_PROFILES + qs;
	 _getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then(function (token) {
			 _handleRequest(token, GET, url, {} , qs, GET_USER_PROFILES, deferred);
	 }).catch(deferred.reject);
	 return deferred.promise;
};

/**
 * @param {string=} uuid, id of the user profile
 * @param {string=} userData, data to update
 * @param {string=} iamToken, optional, if passed request to the server will use this token.
 */
SelfServiceManager.prototype.updateUserProfile = function (uuid, userData, iamToken) {
	const deferred = Q.defer();
	let url = this.managementUrl + MGMT_USER_PROFILES + '/' + uuid + '/profile';
	_getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then(function (token) {
		_handleRequest(token, PUT, url, userData, {}, UPDATE_USER_PROFILE, deferred);
	}).catch(deferred.reject);
	return deferred.promise;
};
